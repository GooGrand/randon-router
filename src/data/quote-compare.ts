...
const toGasPriceTokenIn = (rawValue: string) => {
...
const callBlazingTokenPrice = async (tokenIn: Token, tokenOut: Token, amountIn: string) => {
...
const callBlazingNew = async (
  ...
  const sources = Array.isArray(res.route)
    ? res.route.map((route: { venue_type: string }) => route.venue_type)
    : []

  return {
    aggregator: chunkNumber === null ? 'Blazing Default' : `Blazing chunks ${chunkNumber}`,
    amountOut: res.amount_out_estimated ? String(res.amount_out_estimated) : '0',
    gasUsed: Number(res.gas_used ?? 0),
    sources,
  }
}

const kyberswap = async (tokenIn: Token, tokenOut: Token, amountIn: string): Promise<RawQuote> => {
  ...
  const routes = Array.isArray(summary.route) ? summary.route : []
  const sources = Array.from(
    new Set(
      routes.flatMap((path: any[]) =>
        path.map((part) => (typeof part.poolType === 'string' ? part.poolType : '')),
      ),
    ),
  ).filter(Boolean)
...
const zeroEx = async (tokenIn: Token, tokenOut: Token, amountIn: string): Promise<RawQuote> => {
...
const matcha = async (tokenIn: Token, tokenOut: Token, amountIn: string): Promise<RawQuote> => {
...
const inch = async (tokenIn: Token, tokenOut: Token, amountIn: string): Promise<RawQuote> => {
...
const settleQuotes = async (promises: Promise<RawQuote>[]) => {
...
const calculateScores = (
  ...
  const amountOutValues = raw.map((item) => Number(item.amountOut))
  const gasUsedValues = raw.map((item) => Number(item.gasUsed))
  ...
  const withNet = raw.map((item) => {
    ...
    const gasCost = Number(item.gasUsed) * gasPriceTokenIn
    const netOutput = tokenAmountNumber
      ? amountOut - (gasCost * amountOut) / tokenAmountNumber
      : amountOut
  ...
  return withDistance.map((item) => {
    const amountOutNorm =
      amountOutMax === amountOutMin
        ? 0
        : (Number(item.amountOut) - amountOutMin) / (amountOutMax - amountOutMin)
    const gasCostNorm =
      gasUsedMax === gasUsedMin
        ? 0
        : (Number(item.gasUsed) - gasUsedMin) / (gasUsedMax - gasUsedMin)
...
export const getQuoteComparison = createServerFn({
  ...
  const baseQuotes = await settleQuotes([
    kyberswap(tokenIn, tokenOut, tokenAmount),
    inch(tokenIn, tokenOut, tokenAmount),
    matcha(tokenIn, tokenOut, tokenAmount),
    zeroEx(tokenIn, tokenOut, tokenAmount),
  ])

  const chunkQuotes = await settleQuotes(
    chunkSizes.map((chunk) =>
      callBlazingNew(tokenIn, tokenOut, tokenAmount, chunk, disablePrice),
    ),
  )

  const defaultQuote = await settleQuotes([
    callBlazingNew(tokenIn, tokenOut, tokenAmount, null, disablePrice),
  ])
